...
class RefundInstallmentOrder implements ShouldQueue
{
    ...
    {   
        ...
        if($this->order->payment_method !== 'installment' || !$this->order->paid_at || $this->order->refund_status !== 'REFUND_STATUS_PROCESSING'){
            return;
        }
        ...
        foreach ($installment->items as $item) {
            ...
            if(!$item->paid_at || in_array($item->refund_status, [
                Installment::REFUND_STATUS_SUCCESS,
                Installment::REFUND_STATUS_PROCESSING
            ])){
                continue;
            }
            try {
